...
MODE_RECORD = config['control_mode_record']
...
mem_name = 'shm_record'
mem_config = configs_sm[mem_name]
shm_array, _ = sm_manager.load_memory(mem_name, tuple(mem_config["size"]), mem_config["dtype"])

mem_name = 'shm_servo'
mem_config = configs_sm[mem_name]
shm_servo, _ = sm_manager.load_memory(mem_name, tuple(mem_config["size"]), mem_config["dtype"])

mem_name = 'shm_reward'
mem_config = configs_sm[mem_name]
shm_reward, _ = sm_manager.load_memory(mem_name, tuple(mem_config["size"]), mem_config["dtype"])
...
servo_positions = [0.8, 0.6, 0.3, 0, 0, -0.3, -0.6, -0.8]
        ...
        def update():
            ...
            if events is not None: 
                ...
                if pol_events is not None: # store if not None
                    ...
                    #print(f'num events: {len(pol_events)}')
                    
                    # Check if the trajectory must be recorded
                    if status == 1:
                        print("start Ball")
                        #self.trajectory['data'].append(pol_events.tolist())
                        self.trajectory['rows'].append(filtered_rows)
                    else:
                        if status == 2:
                            print("end Ball")
                            if len(self.trajectory['rows']) > 0:
                                print("store Ball")
                                
                                self.trajectory['background_color'] = background_color
                                self.trajectory['speed'] = speed
                                self.trajectory['trajectory_type'] = trajectory_type
                                self.trajectory['position'] = position
                                
                                file_manager.append(copy.deepcopy(self.trajectory))
                                # store json
                    ...
                    #if len(self.batch_input) == 10:
                    #    shared_array = np.ndarray((batch_size,array_size_in), dtype=dtype, buffer=self.read_shm.buf)
                    #    shared_array[:] = self.batch_input
            ...
            # leep row 1 and 6
            #self.img_red = np.concatenate((self.img_red[1,:], self.img_red[6,:]), axis=0)
                
                # if in_to_out_mapping == 1:
                #     xx = int(((xx/128)*array_size_out))
                # if xx >= excluded_side_pixels and xx < array_size_in + excluded_side_pixels:
                #     xx = xx - excluded_side_pixels
                #     self.img_pos[0,xx]=1
